...
namespace AnalysisTests
{
    ...
    public class Tests
    {
        ...
        //
        // SIF x-x vif      vif 2-2 SIF
        // SIF x-x lsk      lsk 3-3 SIF
        // SIF 3-2 hbk      hbk 1-3 SIF
        // 
        // vif 0-0 lsk      lsk 5-1 vif
        // vif 1-2 hbk      hkb 0-1 vif
        // 
        ...
        private void SetupTeamsForAnalysis()
        {
            ...
            SIF = new SimpleTeam { TeamName = "Strømsgodset", MatchesLeft = 3, PointsBeforeAnalysis = 8, PositionBeforeAnalysis=2 };
            hbk = new SimpleTeam { TeamName = "Hønefoss", MatchesLeft = 2, PointsBeforeAnalysis = 6, PositionBeforeAnalysis=3 };
            vif = new SimpleTeam { TeamName = "vaalerengen", MatchesLeft = 2, PointsBeforeAnalysis = 5, PositionBeforeAnalysis=4 };
            lsk = new SimpleTeam { TeamName = "lillestrøm", MatchesLeft = 2, PointsBeforeAnalysis = 5, PositionBeforeAnalysis=5 };
        ...
        public void SomeEvilTest()
        {
            ...
            foreach (var team in teams)
            {
                var bundles = from b in res
                              where b.Team.TeamName == team.TeamName && b.Outcome == PositionOutcome.Possible
                              select b;

                Console.Write("{0}: ", team.TeamName);
                foreach (var bundle in bundles)
                {
                    Console.Write("{0} ", bundle.Position);
                }
                Console.WriteLine("");
            }
        }
